...
namespace ClassicAssist.Launcher.ViewModels
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, PropertyInfo[]> _commandProperties = new ConcurrentDictionary<Type, PropertyInfo[]>();
        ...
        private static PropertyInfo[] FindCommandProperties( Type type )
        {
            return type.GetProperties( BindingFlags.Public | BindingFlags.Instance )
                .Where( p => typeof( ICommand ).IsAssignableFrom( p.PropertyType ) && p.GetIndexParameters().Length == 0 )
                .ToArray();
        }
    }
