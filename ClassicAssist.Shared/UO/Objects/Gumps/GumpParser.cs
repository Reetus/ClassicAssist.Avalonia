...
public static class GumpParser
{
    ...
    public static Gump Parse( int serial, uint ID, int x, int y, string layout, string[] text )
    {
        ...
        List<GumpPage> pageList = [];
        List<GumpElement> gumpElementList = [];
        ...
        string[] split = layout[layout.IndexOf( '{' )..].TrimEnd( '}', ' ' )
        ...
        for ( int i = 0; i < split.Length; i++ )
        {
            ...
            switch ( formatted[0].ToLower() )
            {
                ...
                case "page":
                    ...
                    {
                        currentPage.GumpElements = [.. gumpElementList];
                    ...
                    try
                    {
                        if ( int.TryParse( formatted[1], out int tpX ) &&
                             int.TryParse( formatted[2], out int tpY ) && int.TryParse(
                                 formatted[3].Split( [','], StringSplitOptions.RemoveEmptyEntries )?[0] ??
                                 "0", out int tpID ) )
                        {
                            GumpElement ge = new()
                            {
                ...
                case "itemproperty":
                    ...
                    if ( int.TryParse( formatted[1], out int itemserial ) )
                    {
                        GumpElement ge = new() { Type = ElementType.itemproperty, Serial = itemserial };
                        gumpElementList.Add( ge );
                    }
        ...
        Gump g = new( x, y, ID, serial, layout, text, [.. gumpElementList], [.. pageList] )
        {
            Closable = closable,
