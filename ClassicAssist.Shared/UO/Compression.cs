...
public static class Compression
{
    ...
    private static readonly Lock _compressLock = new();
    private static readonly Lock _decompressLock = new();
    ...
    private static IEnumerable<string> CandidateRuntimeIdentifiers()
    {
        ...
        foreach ( string directory in Directory.GetDirectories( runtimes ) )
        {
            ...
            if ( dash < 0 || name[( dash + 1 )..].Equals( architecture, StringComparison.OrdinalIgnoreCase ) )
            {
                yield return name;
            }
        }
