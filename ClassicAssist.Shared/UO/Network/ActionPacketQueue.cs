...
public static class ActionPacketQueue
{
    ...
    private static readonly ThreadPriorityQueue<BaseQueueItem> _actionPacketQueue = new( ProcessActionPacketQueue );
    ...
    ///         of the <see cref="DROP_DELAY" /> below. Long enough for the player to have moved in
    ///         between, which invalidates the ground coordinates computed against their old position and
    ///         gets the drop rejected - the item bounces back.
    ///     </para>
    /// </summary>
    ...
    {
        ActionQueueItem actionQueueItem = new( param =>
        {
        ...
        CancellationToken cancellationToken = default, bool checkUseObjectQueue = false, [CallerMemberName] string caller = "" )
    {
        ...
        foreach ( BasePacket packet in packets )
        {
            ...
            PacketQueueItem packetQueueItem = new( data, data.Length, delaySend, caller ) { Token = cancellationToken };
        ...
        CancellationToken cancellationToken = default, bool checkUseObjectQueue = false, [CallerMemberName] string caller = "" )
    {
        ...
        ActionQueueItem actionQueueItem =
            new( e => !cancellationToken.IsCancellationRequested && action( (T) e ) )
            {
                DelaySend = delaySend,
