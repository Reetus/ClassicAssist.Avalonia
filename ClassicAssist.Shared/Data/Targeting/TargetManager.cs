...
public class TargetManager
{
    ...
    private static readonly List<Mobile> _ignoreList = [];
    ...
    private TargetManager()
    {
        ...
        _bodyData = [.. JsonConvert.DeserializeObject<TargetBodyData[]>( File.ReadAllText( Path.Combine( dataPath, "Bodies.json" ) ) )];
    }
        ...
        TargetInfliction inflictionType = TargetInfliction.Any, int maxDistance = -1 )
    {
        ...
        switch ( bodyType )
        {
            ...
            case TargetBodyType.Both:
                bodyTypePredicate = i =>
                    _bodyData.Where( bd =>
                            bd.BodyType is TargetBodyType.Humanoid or TargetBodyType.Transformation )
        ...
        while ( true )
        {
            ...
            switch ( bodyType )
            {
                ...
                case TargetBodyType.Both:
                    bodyTypePredicate = i =>
                        _bodyData.Where( bd =>
                                bd.BodyType is TargetBodyType.Humanoid or
                                  TargetBodyType.Transformation )
                            .Select( bd => bd.Graphic ).Contains( i );
                    break;
                default:
                    throw new ArgumentOutOfRangeException( nameof( bodyType ), bodyType, null );
            ...
            if ( previousMobile != null )
            {
                mobiles = [.. mobiles.Where( m => m.Serial != previousMobile.Serial ).OrderBy( m =>
                    Math.Max( Math.Abs( m.X - previousMobile.X ), Math.Abs( m.Y - previousMobile.Y ) ) )];

                if ( mobiles.Length == 0 )
                {
                    mobiles = [previousMobile];
                }
            }

            if ( reverse )
            {
                mobiles = [.. mobiles.Reverse()];
            }
    ...
    public static Mobile[] ByInflication( this IEnumerable<Mobile> mobiles, TargetInfliction inflictionType )
    {
        switch ( inflictionType )
        {
            ...
            case TargetInfliction.Lowest:
                return [.. mobiles.Where( m => m.Hits < m.HitsMax && !m.IsDead ).OrderBy( m => m.Hits )];
            case TargetInfliction.Poisoned:
                return [.. mobiles.Where( m => m.IsPoisoned )];
            case TargetInfliction.Mortaled:
                return [.. mobiles.Where( m => m.IsYellowHits )];
            case TargetInfliction.Paralyzed:
                return [.. mobiles.Where( m => m.IsFrozen )];
            case TargetInfliction.Dead:
                return [.. mobiles.Where( m => m.IsDead )];
            case TargetInfliction.Unmounted:
                return [.. mobiles.Where( m => m.GetLayer( Layer.Mount ) == 0 )];
            default:
