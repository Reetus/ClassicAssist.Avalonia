...
public class DressAgentEntry : HotkeyEntry
{
    ...
    {
        List<DressAgentItem> list = [.. Items];
    ...
    public async Task Dress( CancellationToken cancellationToken, bool moveConflicting = true )
    {
        ...
        if ( DressManager.GetInstance().UseUO3DPackets )
        {
            ...
            int[] dressItems = [.. Items.Select( i => i.Serial )];
    ...
    public async Task Undress( CancellationToken cancellationToken = default )
    {
        ...
        if ( DressManager.GetInstance().UseUO3DPackets )
        {
            int[] layers = [.. Engine.Player.GetEquippedItems().Where( i => serials.Contains( i.Serial ) ).Select( i => (int) i.Layer )];
