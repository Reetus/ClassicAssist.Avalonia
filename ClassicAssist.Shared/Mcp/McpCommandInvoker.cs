...
public static class McpCommandInvoker
{
    ...
    private static bool TryConvert( JToken value, Type targetType, out object converted, out string error )
    {
        ...
        {
            converted = value.Type == JTokenType.String ? value.ToObject<string>() : value.ToString();
        ...
        if ( underlying.IsEnum )
        {
            try
            {
                converted = Enum.Parse( underlying, raw, true );
                return true;
            }
            catch
            {
        ...
        if ( underlying == typeof( uint ) )
        {
            ...
            {
                if ( uint.TryParse( raw.AsSpan( 2 ), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out uint uintValue ) )
                {
                    converted = uintValue;
                    return true;
                }
        ...
        if ( underlying == typeof( long ) )
        {
            if ( raw.StartsWith( "0x", StringComparison.OrdinalIgnoreCase ) )
            {
                if ( long.TryParse( raw.AsSpan( 2 ), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out long longValue ) )
                {
                    converted = longValue;
                    return true;
                }
