...
public static class McpServer
{
    ...
    private static byte[] Process( byte[] request )
    {
        ...
        {
            return HttpResponse( 405, "Method Not Allowed", [], "Allow: POST, OPTIONS\r\n" );
        }
    ...
    private static JsonRpcResponse HandleToolsCall( JsonRpcRequest request )
    {
        ...
        catch ( Exception e )
        {
            ...
            {
                IsError = true,
                Content = [new McpContent { Text = e.Message }]
            } );
        }
    }
    ...
    private static int ParseContentLength( byte[] data, int headerEnd )
    {
        ...
        foreach ( string line in header.Split( ["\r\n"], StringSplitOptions.None ) )
        {
            ...
            {
                if ( int.TryParse( line.AsSpan( 15 ).Trim(), out int contentLength ) )
                {
