...
public static class McpAgentTools
{
    ...
    public static IReadOnlyList<McpTool> GetTools()
    {
        return new List<McpTool>
        {
            ...
            {
                Name = "listAgents",
                Description = "List the agents (dress, organizer, friends, vendor buy, scavenger, autoloot) with their entry counts.",
                InputSchema = McpTools.ObjectSchema()
            },
            new()
            {
                Name = "getAgent",
                Description = "Get the current entries of an agent so you can query them (e.g. check if a mobile serial is on the friends list). Agents: dress, organizer, friends, vendorbuy, scavenger, autoloot.",
                InputSchema = McpTools.ObjectSchema(
                    new JObject
                    {
                        ["agent"] = McpTools.StringProperty( "The agent name (dress, organizer, friends, vendorbuy, scavenger, autoloot)." ),
    ...
    private static string ListAgents()
    {
        ...
        {
            int count = McpTools.OnUi( () => GetEntryCount( agent ) );
    ...
    private static string GetAgent( string agent, string filter )
    {
        ...
        JArray entries = McpTools.OnUi( () => GetEntries( agent, filter ) );
