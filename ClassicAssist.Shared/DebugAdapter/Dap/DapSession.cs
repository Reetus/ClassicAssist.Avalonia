...
public sealed class DapSession
{
    ...
    private readonly VariableInspector _variableInspector = new();
    ...
    private async Task ReadLoop( CancellationToken token )
    {
        ...
        while ( !token.IsCancellationRequested && !_closed && _stream != null )
        {
            ...
            while ( TryExtractMessage( pending, out byte[] messageBytes ) )
            {
                try
                {
    ...
    private static bool TryExtractMessage( MemoryStream pending, out byte[] messageBytes )
    {
        ...
        foreach ( string line in headerStr.Split( ["\r\n"], StringSplitOptions.None ) )
        {
            if ( line.StartsWith( "Content-Length:", StringComparison.OrdinalIgnoreCase ) )
            {
                ...
                if ( int.TryParse( line[15..].Trim(), out int len ) )
                {
                    contentLength = len;
                }
            }
    ...
    private DapResponse HandleConfigurationDone( DapRequest request )
    {
        if ( _pendingLaunch && !string.IsNullOrEmpty( _launchedProgram ) )
        {
            ...
            if ( macro != null )
            {
                ...
                {
                    macroArgs = [.. _launchedArgs.Split( [','], StringSplitOptions.RemoveEmptyEntries )
                        .Select( a => a.Trim() )
                        .Cast<object>()];
                }
    ...
    private DapResponse HandleSetBreakpoints( DapRequest request )
    {
        ...
        BreakpointInfo[] infos = [.. ( args.Breakpoints ?? [] ).Select( b => new BreakpointInfo
        {
            Line = b.Line,
    ...
    {
        DapThread[] threads = [.. _debugManager.GetThreads().Select( t => new DapThread { Id = t.Item1, Name = t.Item2 } )];
    ...
    private DapResponse HandleSetExceptionBreakpoints( DapRequest request )
    {
        ...
        _debugManager.SetExceptionBreakpoints( args?.Filters ?? [] );
        return MakeResponse( request );
    }
