...
namespace ClassicAssist.Avalonia.Views.Macros
{
    ...
    public partial class MacrosDebuggerControl : UserControl
    {
        ...
        public ICommand ResumeCommand => _resumeCommand ??= new RelayCommand( _ => MacroEntry?.Resume() );

        public ICommand StepCommand => _stepCommand ??= new RelayCommand( _ => MacroEntry?.Step() );

        public ICommand StopCommand => _stopCommand ??= new RelayCommand( _ => MacroEntry?.Stop() );
        ...
        private void OnVariablesGridDoubleTapped( object sender, TappedEventArgs e )
        {
            if ( !( sender is DataGrid grid ) ||
                 !( grid.SelectedItem is KeyValuePair<string, object> kvp ) || !( kvp.Value is Entity entity ) )
            {
                return;
            }
        ...
        private async void OnVariablesGridKeyDown( object sender, KeyEventArgs e )
        {
            ...
            if ( !( sender is DataGrid grid ) || !( grid.SelectedItem is KeyValuePair<string, object> kvp ) )
            {
                return;
            }
